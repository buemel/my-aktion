...
import de.dpunkt.myaktion.util.LogTypes;
...
public class SpendeListTest {
	...
	public static Archive<?> createTestArchive() {
		return ShrinkWrap
				...
				.addClasses(Aktion.class, Konto.class, Spende.class,
						...
						LogTypes.class, TestDataFactory.class)
	...
	public void testBisherGespendet() throws Exception {
		...
		// Überprüfen, ob der gespendete Wert für die Aktion der zweifache
		// Spendenbetrag ist
		Double bisherGespendet = persistedAktion.getBisherGespendet();
		Assert.assertEquals(new Double(2 * TestDataFactory.createTestSpende()
				.getBetrag()), bisherGespendet);
	}
	...
	public void testAddSpende() throws Exception {
		...
		// Überprüfen, ob nach der Spende eine Liste von
		// Spende-Objekten für die Aktion existiert
		List<Spende> spenden = spendeService.getSpendeList(aktion.getId());
		Assert.assertNotNull(spenden);
		// Überprüfen, ob der Betrag der Spende in der Liste
		// derselbe ist, wie in der gemockten Spende
		Assert.assertEquals(TestDataFactory.createTestSpende().getBetrag(),
				spenden.get(0).getBetrag());
	}
}
